...
const requiredSchema = Joi.alternatives(
  ...
  Joi.object({
    ...
    source: Joi.string()
      .valid('label', 'application', 'unavailable')
      .required(),
  }),
);
...
const fieldSchema = Joi.object({
  ...
  find: Joi.string()
    .valid('from_expected', 'fixed_text', 'from_list', 'by_format', 'none')
    .required(),
...
const rulesSchema = Joi.object({
  type: Joi.string()
    .valid('wine', 'distilled-spirits', 'malt-beverage')
    .required(),
...
function assembleField(
  ...
  if (raw.find === 'fixed_text') {
    ...
    if (!ref) {
      throw new Error(
        `Invalid rules data in ${source}: field "${raw.field}" uses fixed_text but names no fixed_text`,
      );
    }
    const fixedText = resources.fixedTexts[ref];
    if (!fixedText) {
      throw new Error(
        `Invalid rules data in ${source}: field "${raw.field}" references unknown fixed_text "${ref}"`,
      );
    }
  ...
  if (raw.find === 'from_list') {
    ...
    if (!ref) {
      throw new Error(
        `Invalid rules data in ${source}: field "${raw.field}" uses from_list but names no list`,
      );
    }
    const list = resources.lists[ref];
    if (!list) {
      throw new Error(
        `Invalid rules data in ${source}: field "${raw.field}" references unknown list "${ref}"`,
      );
    }
  ...
  if (raw.find === 'by_format' && !raw.format) {
    throw new Error(
      `Invalid rules data in ${source}: field "${raw.field}" uses by_format but names no format`,
    );
  }
...
function loadDesignations(file: string): Designation[] {
  const raw = parse<RawDesignation[]>(
    designationsSchema,
    loadYamlFile(file),
    file,
  );
...
export function loadSpiritsRules(): RulesForType {
  ...
  return buildRulesForType(
    loadYamlFile('spirits.rules.yaml'),
    resources,
    'spirits.rules.yaml',
  );
}
