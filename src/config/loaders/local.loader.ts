...
function applyDatabaseEnvOverrides(existing: unknown): Record<string, unknown> {
  ...
  if (env.DB_HOST) database.host = env.DB_HOST;
  if (env.DB_PORT) database.port = Number(env.DB_PORT);
  if (env.DB_NAME) database.name = env.DB_NAME;
  if (env.DB_USER) database.user = env.DB_USER;
  if (env.DB_PASSWORD) database.password = env.DB_PASSWORD;
  return database;
}
