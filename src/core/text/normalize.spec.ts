...
describe("normalizedText", () => {
  it("makes case and punctuation differences equal", () => {
    expect(normalizedText("STONE'S THROW")).toBe(normalizedText("Stone's Throw"));
  });
});
...
describe("containsPhrase", () => {
  it("finds a contiguous run of words", () => {
    expect(containsPhrase("Bottled by Old Tom Distillery, Bardstown, KY", "Old Tom Distillery, Bardstown, KY")).toBe(true);
  });
  ...
  it("requires the words in order", () => {
    expect(containsPhrase("Distillery Old Tom", "Old Tom Distillery")).toBe(false);
  });
});
